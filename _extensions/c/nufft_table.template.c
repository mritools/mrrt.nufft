...
TYPE *i_fm)
{
    ...
    for (jj1=0; jj1 < J1; jj1++, k1++) {
        ...
        register const TYPE coef1r = r_h1[n1];
        register const TYPE coef1i = i_h1[n1];
...
const int N)
{
    ...
    for (nn=0; nn<N; nn++) {
       CAT(TYPE, _interp1_table0_complex_per_adj_inner)(&r_ck[nn*K], &i_ck[nn*K],
                        K1,
                        r_h1, i_h1, 
                        J1, L1, 
...
const int N)
{
    ...
    #pragma omp parallel for private(nn)
    for (nn=0; nn<N; nn++) {
       CAT(TYPE, _interp2_table0_complex_per_adj_inner)(&r_ck[nn*K], &i_ck[nn*K],
                        K1, K2, 
                        r_h1, i_h1, r_h2, i_h2,
                        J1, J2, L1, L2, 
                        p_tm, M, &r_fm[nn*M], &i_fm[nn*M]);
...
TYPE *i_fm)
{
    ...
    for (jj3=0; jj3 < J3; jj3++, k3++) {
        ...
        const TYPE coef3r = r_h3[n3];
        const TYPE coef3i = i_h3[n3];
...
const int N)
{
    ...
    #pragma omp parallel for private(nn)
    for (nn=0; nn<N; nn++) {
       CAT(TYPE, _interp3_table0_complex_per_adj_inner)(&r_ck[nn*K], &i_ck[nn*K],
                        K1, K2, K3, 
